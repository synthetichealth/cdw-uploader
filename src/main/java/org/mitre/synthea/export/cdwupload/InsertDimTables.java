...
public class InsertDimTables {
	...
	public static void load(Connection cn, String dir, Boolean batchMode) throws Exception{	
		...
		TableInsert.load(dir+"localdrug.csv",cn,"Dim.LocalDrug",batchMode,
				"(LocalDrugSID, LocalDrugIEN, Sta3n, LocalDrugNameWithDose, NationalDrugSID, NationalDrugNameWithDose, PharmacyOrderableItemSID )"
				,F.s, F.s, F.s, F.s, F.s,
				F.s , F.s);
		TableInsert.load(dir+"location.csv",cn,"Dim.Location",batchMode," ( LocationSID,LocationName) ",F.s,
				F.qt(50) //)F.quoteTrunc.apply(50) //F.g
				);
		...
		TableInsert.load(dir + "sta3n.csv", cn, "Dim.Sta3n", batchMode,"(Sta3n,Sta3nName,TimeZone)", F.s	
				,F.qt(50) //,F.quoteTrunc.apply(50)//,F.g  
				,F.s );
